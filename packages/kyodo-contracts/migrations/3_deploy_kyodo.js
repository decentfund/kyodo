...
module.exports = async (deployer, network, accounts) => {
  ...
  const kyodoDeployed = await deployer.deploy(KyodoDAO);

  // Setting new proxy
  const kyodoProxy = await OwnedUpgradeabilityProxy.new();

  // Initializing proxy with data
  await kyodoProxy.upgradeToAndCall(kyodoDeployed.address, initializeData);
  ...
  // Getting deployed token address
  const tokenDeployed = await Token.deployed();

  // Setting token address for proxy
  await kyodoProxyInstance.setTokenAddress(tokenDeployed.address);
